...
if( $_SERVER[ 'REQUEST_METHOD' ] == 'POST' ) {
    ...
    // echo $where_part; exit ;
        ...
        $countOfGroups = 0;
        $sqlForCount = "SELECT COUNT(*) FROM product_groups";
        try { 
            $table = $_CONTEXT[ 'connection' ]->query( $sqlForCount  ) ;
            $countOfGroups = $table->fetch( PDO::FETCH_ASSOC ) ;
        }
        catch( PDOException $ex ) {
            $_CONTEXT['logger']( 'shop_controller4 ' . $ex->getMessage() . $sqlForCount ) ;
        ...
        if( count( $filters[ 'product_groups_id' ] ) > 0 && count( $filters[ 'product_groups_id' ] ) !== (int)$countOfGroups["COUNT(*)"]) {  // AND p.id_grp IN ( '111','222','333' )
            $where_part .=  
                ( ($where_part == "") ? " WHERE " : " AND " )
